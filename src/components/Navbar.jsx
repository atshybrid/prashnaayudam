    ...
    const renderSocial = () => (
        <>
            <div className='bg-white p-2 shadow-sm rounded-full cursor-pointer'>
                <BsFacebook className={`size-6 ${color ? '' : 'text-[#2E3192]'}`} style={{ color: color ? color : undefined }} />
            </div>
            <div className='bg-white p-2 shadow-sm rounded-full cursor-pointer'>
                <BsInstagram className={`size-6 ${color ? '' : 'text-[#2E3192]'}`} style={{ color: color ? color : undefined }} />
            </div>
            <div className='bg-white p-2 shadow-sm rounded-full cursor-pointer'>
                <BsTwitter className={`size-6 ${color ? '' : 'text-[#2E3192]'}`} style={{ color: color ? color : undefined }} />
            </div>
            <div className='bg-white p-2 shadow-sm rounded-full cursor-pointer'>
                <BsYoutube className={`size-6 ${color ? '' : 'text-[#2E3192]'}`} style={{ color: color ? color : undefined }} />
            </div>
        </>
    );

    return (
        <nav className={`${color ? '' : 'bg-[#2E3192]'} border-gray-200 p-4`} style={color ? { backgroundColor: color } : {}}>
                    ...
                    <div className='hidden xl:flex flex-1 padding-r justify-between'>
                        <ul className="flex space-x-8">
                            <div className='flex items-center py-4 cursor-pointer' onClick={() => toggleDropdown(i)}>
                                <span className="text-lg font-medium text-white uppercase">{'Main Editions'}</span>
