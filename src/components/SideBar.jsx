            ...
            >
                <section className="bg-white flex flex-col absolute left-0 top-0 h-screen min-w-[280px]">
                    <div className='flex h-16 px-2 items-center justify-between border-b border-gray-400'>
                        <img src={settings?.sitelogo || epaper_logo} alt="Site Logo" className='h-12 object-contain' />
                        <BsX
                    ...
                    <div className="flex flex-col ">
                        {data?.map((item, i) => (
                            <Link key={i} className={`text-slate-500 ${color ? '' : 'hover:text-[#2E3192]'}  uppercase font-semibold hover:font-bold p-4 border-b border-gray-400`} onClick={item.link}>
                                {item.label}
                            </Link>
                        ))}
                    </div>
